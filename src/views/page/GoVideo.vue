  ...
  <div class="video-container">
    ...
    <div class="video-card">
      ...
      <div v-else class="video-player-wrapper">
        <vue-plyr ref="plyr" :options="options" @player="onPlayerReady">
...
export default {
  ...
  data() {
    return {
      ...
      subtitle: "",
      suffix: "",
      plyrInstance: null, // Lưu giữ instance chuẩn của Plyr
    };
  },
  mounted() {
    this.render();
  ...
  methods: {
    // Callback được gọi ngay khi Vue-Plyr khởi tạo xong
    onPlayerReady(player) {
      this.plyrInstance = player;
      this.initMobileFullscreenFix(player);

      // Nếu là HLS/FLV cần gắn media element sau khi player sẵn sàng
      if (this.suffix === "m3u8") {
        this.loadHls({ src: this.videoUrl, autoplay: this.options.autoplay, media: player.media });
      } else if (this.suffix === "flv") {
        this.loadFlv({ src: this.videoUrl, autoplay: this.options.autoplay, media: player.media });
      }
    },

    // Kích hoạt chế độ phóng to chuẩn thiết bị trên điện thoại (đặc biệt là iOS Safari)
    initMobileFullscreenFix(player) {
    ...
    render() {
      ...
      this.suffix = path.substring(index + 1).toLowerCase();
    ...
    loadHls(options) {
      import("@/plugin/vplayer/hls").then((res) => {
        ...
        Hls({
          ...
          callback: (hls) => {
            if (this.plyrInstance) {
              this.plyrInstance.on("languagechange", () => {
                setTimeout(
                  () => (hls.subtitleTrack = this.plyrInstance.currentTrack),
                  50
                );
              });
            }
  ...
  computed: {
    ...
    title() {
      // Tự động tách lấy tên file từ đường dẫn URL nếu không có prop title
    ...
    players() {
      ...
      return [
        {
          name: "IINA",
          icon: this.$cdnpath ? this.$cdnpath("images/player/iina.png") : "",
          scheme: "iina://weblink?url=" + this.videoUrl,
        },
        {
          name: "PotPlayer",
          icon: this.$cdnpath ? this.$cdnpath("images/player/potplayer.png") : "",
          scheme: "potplayer://" + this.videoUrl,
        },
        {
          name: "VLC",
          icon: this.$cdnpath ? this.$cdnpath("images/player/vlc.png") : "",
          scheme: "vlc://" + this.videoUrl,
        },
        {
          name: "Thunder",
          icon: this.$cdnpath ? this.$cdnpath("images/player/thunder.png") : "",
          scheme: "thunder://" + this.getThunder,
        },
        {
          name: "Aria2",
          icon: this.$cdnpath ? this.$cdnpath("images/player/aria2.png") : "",
          scheme: 'javascript:alert("Chức năng đang phát triển")',
        },
        {
          name: "nPlayer",
          icon: this.$cdnpath ? this.$cdnpath("images/player/nplayer.png") : "",
          scheme: "nplayer-" + this.videoUrl,
        },
        {
          name: "MXPlayer (Free)",
          icon: this.$cdnpath ? this.$cdnpath("images/player/mxplayer.png") : "",
          scheme:
            "intent:" +
            this.videoUrl +
    ...
    getThunder() {
      ...
      return btoa("AA" + this.videoUrl + "ZZ");
    },
  },
};
</script>
...
:deep(.plyr--fullscreen-active video),
:deep(.plyr--fullscreen-fallback video),
